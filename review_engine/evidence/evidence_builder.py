...
class EvidenceBuilder:
    ...
    def build(self, snapshot: Mapping[str, object]) -> dict[str, object]:
        ...
        return {"schema_version": self.SCHEMA_VERSION, "producer": "RAIP Review & Evidence Engine", "owner": "RAIP Evidence Layer", "created_at": self._iso(self.clock()), "evidence_id": evidence_id, "trade_id": trade_id, "snapshot_id": snapshot.get("snapshot_id"), "snapshot_sha256": snapshot_hash, "classification": self.classifier.classify(snapshot), "result": "WIN" if net_profit > 0 else "LOSS" if net_profit < 0 else "BREAKEVEN", "win_loss": "WIN" if net_profit > 0 else "LOSS" if net_profit < 0 else "BREAKEVEN", "rr": rr, "confidence": decision.get("confidence"), "market_state": market.get("market_state") or market.get("market_mode") or "UNKNOWN", "statistics": self.scoring.score(snapshot)}
    @staticmethod
