...
The latest external compile reached **0 errors / 1 warning**. Its sole Warning 43 was `FileSize(handle)` (`ulong`) being stored in `long file_size`. The current correction removes that narrowing conversion, retains the size as `ulong`, and performs the required `int` read-count conversion only after the existing 1 MiB bound proves it range-safe. Final native **0 errors / 0 warnings** confirmation remains pending.
