...
APPLICABILITY_REPORT_CONTRACT_VERSION = "1.0.0"
_SEMVER = re.compile(
...
def semver_major(version: object) -> int | None:
...
class RuntimeContext:
    ...
    def __post_init__(self) -> None:
        ...
        if semver_major(self.runtime_version) is None:
...
class ApplicableKnowledge:
    ...
    def __post_init__(self) -> None:
        if (
            not isinstance(self.knowledge_uuid, str) or not self.knowledge_uuid.strip()
            or not isinstance(self.semantic_identity, str) or not self.semantic_identity.strip()
            ...
            or not isinstance(self.matching_factors, tuple)
            or not isinstance(self.reason_codes, tuple)
        ):
            raise ValueError("INVALID_APPLICABLE_KNOWLEDGE")
        object.__setattr__(self, "knowledge_uuid", self.knowledge_uuid.strip())
        object.__setattr__(self, "semantic_identity", self.semantic_identity.strip())
...
class ApplicabilityReport:
    ...
    report_digest: str = ""
    report_uuid: str = ""
    report_contract_version: str = APPLICABILITY_REPORT_CONTRACT_VERSION

    def __post_init__(self) -> None:
        if (
            not isinstance(self.snapshot_digest, str)
            or not _HEX64.fullmatch(self.snapshot_digest)
            or semver_major(self.report_contract_version) is None
            or isinstance(self.confidence, bool)
            or not isinstance(self.confidence, (int, float))
            or not isfinite(self.confidence)
            or not 0 <= self.confidence <= 1
            or not isinstance(self.context, RuntimeContext)
            or not isinstance(self.applicable, tuple)
            or not isinstance(self.rejected, tuple)
        ):
            raise ValueError("INVALID_APPLICABILITY_REPORT")
        canonical_order = tuple(sorted(self.applicable, key=lambda item: (
            -item.priority, -item.specificity_score, -item.confidence,
            item.semantic_identity, item.knowledge_uuid,
        )))
        if self.applicable != canonical_order:
            raise ValueError("NON_CANONICAL_APPLICABILITY_ORDER")
        if any(item.snapshot_digest != self.snapshot_digest for item in self.applicable):
            raise ValueError("APPLICABILITY_PROVENANCE_MISMATCH")
        body = {
...
class KnowledgeApplicabilityEngine:
    ...
    def evaluate(self, snapshot: KnowledgeRuntimeSnapshot, context: RuntimeContext) -> ApplicabilityReport:
        ...
        for candidate in sorted(candidates, key=lambda item: (item["semantic_identity"], item["knowledge_uuid"])):
            ...
            accepted.append(ApplicableKnowledge(
                candidate["knowledge_uuid"], candidate["semantic_identity"], specificity,
                candidate["priority"], candidate["confidence"], tuple(factors), tuple(codes),
                snapshot.snapshot_digest, candidate["registry_sequence"], candidate["metadata"],
            ))
        ...
        for item in accepted:
            ...
            if old is None or rank < (-old.priority, -old.specificity_score, -old.confidence, old.knowledge_uuid):
        ...
        selected = tuple(sorted(winners.values(), key=lambda item: (
            -item.priority, -item.specificity_score, -item.confidence,
            item.semantic_identity, item.knowledge_uuid,
        )))
    ...
    def _validate_snapshot(self, snapshot: object, context: object) -> None:
        ...
        if semver_major(snapshot.gateway_contract_version) not in self.config.supported_gateway_contract_majors:
            raise ApplicabilityEvaluationError("UNSUPPORTED_GATEWAY_CONTRACT_VERSION")
        if semver_major(snapshot.compatibility_policy_version) not in self.config.supported_policy_majors:
    ...
    def _candidate(entry: RuntimeKnowledgeDescriptor) -> dict[str, Any]:
        ...
        try:
            ...
            if versions != ("*",) and any(semver_major(value) is None for value in versions):
            ...
            return {
                ...
                "registry_sequence": entry.sequence,
                "supported_runtime_versions": versions,
                "required_features": _values(metadata.get("required_features", ()), "INVALID_REQUIRED_FEATURES", True),
    ...
    def _validate_candidates(candidates: list[dict[str, Any]]) -> None:
        ...
        for item in candidates:
            ...
            if item["knowledge_uuid"] in knowledge or item["registry_sequence"] in sequences or identity in identities:
                raise ApplicabilityEvaluationError("DUPLICATE_APPLICABILITY_CANDIDATE")
            knowledge.add(item["knowledge_uuid"])
            sequences.add(item["registry_sequence"])
            identities.add(identity)

    @staticmethod
    def _match(item: dict[str, Any], context: RuntimeContext):
        codes: list[str] = []
        factors: list[str] = []
        exact = 0
        for label, context_name, candidate_name in _MATCH_FIELDS:
            ...
            if allowed == ("*",):
                ...
                codes.append(f"{label}_MATCH")
                factors.append(f"{label}_MATCH")
                exact += 1
            else:
        ...
        elif context.runtime_version in versions:
            codes.append("RUNTIME_VERSION_MATCH")
            factors.append("RUNTIME_VERSION_MATCH")
            exact += 1
        else:
