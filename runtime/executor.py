...
from runtime.broker_safety import BrokerOrderResult, BrokerSafety, ExecutionInstruction, MT5ExecutionInterface, OrderRequest
...
class Executor:
    ...
    def _submit(self, instruction: ExecutionInstruction, execution_id: str, sequence_id: int) -> ExecutionResult:
        ...
        while True:
            attempts += 1
            result: BrokerOrderResult = self._broker.send_order(OrderRequest(instruction, execution_id, f"RP:{execution_id}"))
            if result.accepted:
