...
class RuntimeKnowledgeSelectionPolicy:
    ...
    def __post_init__(self):
        expected = ("ADVISORY_PACKAGE_PREPARED", "ADVISORY_ENTRY_RECORDED",
                    "STATISTICALLY_CONSISTENT", "POLICY_CRITERIA_MET")
        if (not isinstance(self.selection_policy_version,str) or not self.selection_policy_version
                or not isinstance(self.runtime_selector_version,str) or not self.runtime_selector_version
                or (self.minimum_package_integrity, self.required_registry_state,
                    self.required_validation_state, self.required_promotion_state) != expected):
    ...
    @property
    def selection_policy_uuid(self): return policy_uuid(self.to_dict())
    @property
    def selection_policy_digest(self): return digest(self.to_dict())
