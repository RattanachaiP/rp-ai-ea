...
class LifecycleValidator:
    ...
    def validate(self, governance: KnowledgeGovernance, existing: Iterable[KnowledgeGovernance] = ()) -> None:
        required = ("knowledge_uuid", "pattern_uuid", "validation_uuid", "analytics_uuid", "source_baseline_commit", "rule_version", "analytics_version", "lineage_reference")
        ...
        if governance.current_lifecycle_state in {"ACTIVE", "SUPERSEDED", "RETIRED", "ARCHIVED"} and promoted is None:
        ...
        history = sorted((item for item in existing if item.knowledge_uuid == governance.knowledge_uuid), key=lambda item: item.record_version)
        if not history:
        ...
        if governance.record_version != prior.record_version + 1:
            raise GovernanceValidationError("INVALID_RECORD_VERSION_SEQUENCE")
        immutable = ("knowledge_uuid", "pattern_uuid", "validation_uuid", "analytics_uuid", "source_baseline_commit", "rule_version", "analytics_version", "lineage_reference", "schema_version")
