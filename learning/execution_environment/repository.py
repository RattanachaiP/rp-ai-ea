...
from .models import ExecutionEnvironment, ExecutionEnvironmentEvidence, ExecutionEnvironmentSnapshot
...
class ExecutionEnvironmentEvidenceRepository:
    ...
    def records(self):
        ...
        return helper._load(self.root, ExecutionEnvironmentEvidence,
                            "EXECUTION_ENVIRONMENT_EVIDENCE", "evidence_uuid")

    def for_readiness(self, readiness):
        matches = tuple(x for x in self.records() if
            (x.execution_readiness_uuid, x.execution_readiness_digest) ==
