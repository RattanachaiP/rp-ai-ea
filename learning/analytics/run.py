...
def main():
    ...
    analytics_path = Path(args.analytics_path).expanduser().resolve()
    ...
    if not args.no_persist and not args.source_baseline:
        parser.error("--source-baseline is required when persistence is enabled")
    ...
    try:
        ...
        report = KnowledgeAnalyticsEngine(
            KnowledgeReader(KnowledgeRepository(knowledge_path)),
            repository,
            source_baseline=args.source_baseline or "UNKNOWN",
        ).analyze()
