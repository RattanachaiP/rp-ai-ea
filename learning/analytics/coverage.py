...
def coverage(records, config):
    ...
    return {"by_dimension":groups,"low_coverage_conditions":[k for k,v in sorted(condition.items()) if len(v)<config.low_coverage_threshold],"highly_concentrated_conditions":[k for k,v in sorted(condition.items()) if len(v)>=config.concentration_threshold],"duplicate_condition_coverage":[{"condition":k,"knowledge_ids":sorted(v)} for k,v in sorted(condition.items()) if len(v)>1],"uncovered_conditions":None, "uncovered_conditions_status":"NOT_COMPUTED_NO_EXPECTED_UNIVERSE"}
