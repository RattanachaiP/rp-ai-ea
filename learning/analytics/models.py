...
class AnalyticsConfig:
    ...
    maximum_conflict_comparisons: int = 10_000
    ...
    def __post_init__(self) -> None:
        ...
        thresholds = (
            ...
            self.low_sample_threshold,
            self.maximum_findings,
            self.maximum_conflict_comparisons,
        )
        deltas = (
            self.stable_win_rate_delta,
            self.stable_rr_delta,
            self.improving_win_rate_delta,
            self.improving_rr_delta,
            self.conflict_win_rate_delta,
            self.conflict_rr_delta,
        )
        if not self.version or not versions or len(set(versions)) != len(versions):
            raise ValueError("INVALID_ANALYTICS_CONFIG_VERSION")
        if any(not isinstance(value, int) or value <= 0 for value in thresholds):
            raise ValueError("INVALID_ANALYTICS_CONFIG_INTEGER")
        if any(not math.isfinite(value) or value < 0 for value in deltas):
            raise ValueError("INVALID_ANALYTICS_CONFIG_DELTA")
        if (
            self.improving_win_rate_delta < self.stable_win_rate_delta
            or self.improving_rr_delta < self.stable_rr_delta
        ):
            raise ValueError("INVALID_ANALYTICS_CONFIG_BANDS")
...
class AnalyticsReport:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "failure_diagnostics",
            tuple(freeze(item) for item in self.failure_diagnostics),
        )
