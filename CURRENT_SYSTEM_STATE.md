...
- PR185 requires the canonical PR184 repository and snapshot lineage; it cannot create
  truthful intelligence on a completely empty machine. Missing upstream governed state
  is a terminal failure, never an invitation to fabricate a bootstrap record.
...
- The MT5 Common Files root and symbol are explicit operator configuration. The existing Executor startup callback is invoked only after the published context is consumed and Runtime is `READY`.
- Repository discovery found no tracked production MT5 process/EA launcher; PR196 therefore does not invent one. The existing production host supplies its Executor startup callback, and all wiring failures are terminal with no fallback.
...
- Output states are `REJECTED`, `INSUFFICIENT_RECOMMENDATION_EVIDENCE`, `RECOMMENDATION_READY`, `RECOMMENDATION_MANUAL_REVIEW`, and `RECOMMENDATION_NOT_READY`; classifications are `READY_FOR_DECISION`, `NOT_READY`, `INSUFFICIENT_EVIDENCE`, `MANUAL_REVIEW`, and `REJECTED`. None is a trading decision or grants execution authority.
...
- The existing V27 Production Executor emits a signed, immutable acceptance attestation for the
  exact PR278 Executor Admission Authorization. PR279 authenticates that executor-origin evidence;
  it never manufactures acknowledgement.
- Exact admission-entry membership, current upstream PR278 effectiveness/revocation, authorization,
  executor instance/session, artifact/contract/environment/policy/authority lineage, freshness, and
  attestation/nonce replay protections are mandatory and fail closed.
- Success records acknowledgement only through
  `AUTHORIZED → ACKNOWLEDGED → RECORDED → ACKNOWLEDGEMENT_COMPLETE`. Operational execution
...
- PR280 verifies the exact immutable PR279 authorization, evidence, registry membership,
  ...
  a bounded, single-use `RuntimeStartupAuthorization`.
- All 22 gates are critical and deterministic. Successful and rejected outcomes have
  ...
  validation, and generation-aware replay protection. Revocation, supersession, expiry,
  emergency rollback, duplicate authorization, and duplicate consumption fail closed.
