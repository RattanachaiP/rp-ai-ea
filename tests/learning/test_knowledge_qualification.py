...
class AnalyticsProvider:
    def latest(self): return {"analytics_version": "1.0", "configuration_version": "1.0", "status": "READY", "stability_classification": "STABLE", "conflict_severity": "LOW", "timestamp": "2026-07-24T00:00:00Z"}


class PolicyProvider:
    def get_policy_result(self, knowledge_uuid=None): return {"knowledge_uuid": knowledge_uuid, "eligible": True}
...
def _plane(tmp_path):
    ...
    governance.save(KnowledgeGovernance("knowledge-1", "pattern-1", "validation-1", "analytics-1", "a" * 40, "1.0", "1.0", "ACTIVE", "pattern-1/1", True, "2026-07-24T00:00:00Z"))
    ...
    return KnowledgeControlPlane(tmp_path, knowledge_repository=knowledge_repo, governance_repository=governance, lifecycle_repository=lifecycle, analytics_repository=AnalyticsProvider(), policy_provider=PolicyProvider(), policy_configuration_version="1.0"), record


def digest(snapshot):
    body = dict(snapshot); body.pop("snapshot_digest", None)
    snapshot["snapshot_digest"] = hashlib.sha256(json.dumps(body, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
    return snapshot
...
def test_missing_analytics_is_insufficient_information(tmp_path):
    item = snapshot(tmp_path)
...
def test_invalid_snapshot_and_immutable_report(tmp_path):
    item = snapshot(tmp_path); item["snapshot_digest"] = "wrong"
