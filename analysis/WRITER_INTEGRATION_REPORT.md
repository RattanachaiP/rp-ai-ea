...
## Heartbeat policy
`heartbeat_unix` is compared with an injected clock. The default maximum age is 30 seconds and is configurable through `heartbeat_max_age_seconds`. Expired publications are never forwarded.
...
## Executable contract
The bridge treats `executable` exclusively as a publisher-owned boolean. It does not derive executability from decision, confidence, probability, scores, direction, or any other analytical value. `false` stays non-executable.
