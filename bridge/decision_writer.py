...
class DecisionWriter:
    ...
    def __init__(self, decision_path: Path | str, *, heartbeat_max_age_seconds: float = _DEFAULT_HEARTBEAT_MAX_AGE_SECONDS,
        ...
        if heartbeat_max_age_seconds < 0:
            raise ValueError("HEARTBEAT_MAX_AGE_MUST_BE_NON_NEGATIVE")
    ...
    def _validate_heartbeat(self, document: Mapping[str, Any]) -> None:
        if self._clock() - document["heartbeat_unix"] > self.heartbeat_max_age_seconds:
            raise _ValidationError("STALE_HEARTBEAT")

    @staticmethod
    def _validate_executable_contract(document: Mapping[str, Any]) -> None:
        # Executability is a publisher-owned boolean.  The writer does not infer it
        # from decision, direction, confidence, or any analytical field.
        if type(document["executable"]) is not bool:
            raise _ValidationError("MALFORMED_EXECUTABLE")
